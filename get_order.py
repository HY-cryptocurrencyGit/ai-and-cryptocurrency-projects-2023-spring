...
def write_csv(fn, df):
    ...
    if should_write_header == False:
        df.to_csv(fn, index=False, header=True, mode = 'w')
    else:
...
def pull_csv_book_trade():
    ...
    while 1:
        ...
        for ex in _list_ex:
            book_fn = '%s/%s-%s-%s-book.csv'% (csv_dir, req_time, ex, currency)
            ...
            if int(req_timestamp[17:19]) % 60 == 0: print(book_df.iloc[0,-1]) 
